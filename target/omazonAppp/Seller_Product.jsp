    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="add-product" name="addProduct" action="ProductServlet" method="post"
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" required type="text" name="productName" id="productName"
                    ...
                    <div class="form-group">
                        <label> Price</label>
                        <input class="form-control" onmouseout="ValNumber()" required type="text" name="productPrice"
                               id="productPrice" placeholder="Product Price">
                        <div style="display: none; color:red; font-weight: bold" id="invalid-feedback">Please enter
                            correct format (XX.XX). Eg : 10.00
                        </div>
                    </div>

                    <div class="form-group">
                        <label>Description</label>
                        <input class="form-control" required type="text" name="productDescription"
                               id="productDescription" placeholder="Product Description">
                    </div>

                    <div class="form-group">
                        <label>Product Image</label>
                        <input class="form-control" required type="file" name="productImage" id="productImage"
                               accept="image/*">
                    </div>

                    <div class="form-group">
                        <label>Stock Count</label>
                        <input class="form-control" required type="number" id="productStock" name="productStock"
    ...
    <div class="table">
        <table id="table" class="table table-responsive table-hover table-sm table-bordered">
            ...
            <tr>
                ...
                <td>P<%=product.getProductID()%>
                </td>
                <td><%=product.getProductName()%>
                </td>
                <td><img src="pics/<%=product.getProductFilePath()%>" width="100px" class="img-fluid"></td>
                <td><%=product.getProductCategory()%>
                </td>
                <td><%=product.getProductDescription()%>
                </td>
                <td>RM <%=df.format(product.getProductPrice())%>
                </td>
                <td><%=product.getProductStockCount()%>
                </td>
                <td>RM <%=df.format(product.getProductSalesCount() * product.getProductPrice())%>
                </td>
                <td>
                    <button type="button" class=" btn btn-info EDIT" name="button" data-toggle="modal"
                            data-target="#editModal<%=product.getProductID()%>"><i class="far fa-edit"></i></button>
                </td>
                <td>
                    <form id="delete-product-<%=product.getProductID()%>" action="ProductServlet" method="post"
                          novalidate>
                        <input type="hidden" name="productIDToDelete" value="<%=product.getProductID()%>">
                        <button type="submit" id="deletebutton" class=" btn btn-danger delete"
                                value="<%=product.getProductID()%>" name="dltbtn"><i class="fas fa-trash"></i></button>
                    </form>
                </td>
            </tr>
